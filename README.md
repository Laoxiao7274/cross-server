...
插件现在开始支持语言配置与 Web 面板语言切换：

```yaml
language:
  locale: "zh-CN"
  web-default-locale: "zh-CN"
```
...
- Web 面板中英文切换框架
- 部分关键启动日志与高频提示
- Homes / Warps / Auth 第一批玩家高频文本迁移
- TPA / 跨服传送主链路第一批高频文本迁移
- TPA 剩余高频提示、主命令帮助核心文案、总菜单标题第二批迁移
- `/cs` 管理命令高频入口、总菜单 lore/灰显提示、plugin.yml/config 默认文本第三批迁移
- Web 面板主框架（标题、标签页、搜索框、核心按钮、部分成功提示）第四批迁移
- Web 面板页面细节（空态、表格空列表、部分成功提示、模块/路由状态标签）第五批迁移
- 仍有少量管理命令正文和 Web 深层详情文案待继续迁移，后续版本会继续补全直到完全覆盖

后续版本会继续把剩余硬编码文案逐步迁移到统一语言系统中。
...
`/cs menu` 的总菜单也会根据当前权限和模块状态做灰显提示，减少误点和无效入口。
