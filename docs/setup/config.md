...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `locale` | String | `zh-CN` | 插件服务端默认语言 |
| `web-default-locale` | String | `zh-CN` | Web 面板默认语言 |
...
- 这一版已经接入语言配置底座与 Web 切换框架
- 后续版本会继续把剩余用户可见文案逐步迁移到统一语言系统
...
CrossServer 现在会自动记录最近的关键配置变更历史，便于面板回看：
